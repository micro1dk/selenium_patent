...
class UPLOAD_FILES(Browser, PyautoGUI):
    ...
    try:
      ...
      for f in os.listdir(FOLDER_DIR):
        ...
        for complete in complete_list:
          ...
          self.page_search(accept_no, application_no, classify_no, f) # 한줄마다 페이지 전체를 순회하여 검색
          print('complete!')
  ...
  def page_search(self, accept_no, application_no, classify_no, markinfo_acc_no):
    ...
    while True:
      if i > 3:
        self.driver.find_element_by_xpath(f'//*[@id="form"]/div[4]/div/div/ul/li[{i}]/a').click()
      ...
      for j in range(1, number_tr + 1):
        if self.driver.find_element_by_xpath(f'//*[@id="table-view"]/tbody/tr[{j}]').get_attribute('class') != 'template':
          continue
        title_elem = self.driver.find_element_by_xpath(f'//*[@id="table-view"]/tbody/tr[{j}]/td[5]') # 요기안에 제목 들어있음
        reply_elem = self.driver.find_element_by_xpath(f'//*[@id="table-view"]/tbody/tr[{j}]/td[6]') # 회신사항
        state_elem = self.driver.find_element_by_xpath(f'//*[@id="table-view"]/tbody/tr[{j}]/td[7]') # 회신사항
        ...
        if '최종제출동의' == reply_elem.text and '처리대기' in state_elem.text and markinfo_acc_no in title_elem.text and f'{classify_no}류' in title_elem.text:
          ...
          time.sleep(3)
          if i > 3:
            self.driver.find_element_by_xpath('//*[@id="form"]/div[4]/div/div/ul/li[1]/a').click()
          return

      if i == 12:
        success, attr = self.check_attribute(f'//*[@id="form"]/div[4]/div/div/ul/li[{i + 1}]/a', 'disabled')
        if success:
          if attr != 'disabled':
  ...
  def detail_page(self, accept_no, application_no, classify_no, markinfo_acc_no):
    try:
      ...
      number_inputs = self.driver.find_elements_by_class_name('application_number')
      edit_btns = self.driver.find_elements_by_class_name('application_edit')
      submit_btns = self.driver.find_elements_by_class_name('btn_add_file')
      ...
      for i in range(len(bib_classifies)):
        ...
        if bib_classifies[i].get_attribute('value') == classify_no:
          ...
          submit_btns[i].click()
          inputs[i].send_keys(f'{FOLDER_DIR}\\{accept_no}\\1-{classify_no}.pdf\n{FOLDER_DIR}\\{accept_no}\\2-{classify_no}.pdf')
          # 확인 필요
...
def main(driver):
  upload_files = UPLOAD_FILES(driver)
